...
1. **Infraestructura (una vez):** `infra/terraform-aws-lambda/` crea la
   función, su rol IAM, el log group y la Function URL. Ver el README de
   ese directorio.
2. **Código (cada push a `main`):** el workflow `Serverless Deploy - Prod`
   empaqueta el artefacto, valida el handler localmente (invoca `/health`
   y `POST /api/shorten` en memoria antes de desplegar), sube el zip con
   `aws lambda update-function-code` y hace smoke test contra
   `LAMBDA_FUNCTION_URL/health`.
...
|---|---|---|
| secret | `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` | Credenciales del usuario IAM de despliegue (permiso mínimo: `lambda:UpdateFunctionCode`, `lambda:GetFunction` sobre la función) |
...
| var | `LAMBDA_FUNCTION_URL` | Function URL para el smoke test post-deploy |
